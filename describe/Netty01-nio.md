...
```java
// 分配java堆内存，此方法容量无法动态调整，读写效率低，受到GC的影响
Bytebuffer buf = ByteBuffer.allocate(16);
// 分配直接内存，读写效率高（少一次拷贝），不会受GC影响，分配效率低且使用不当的话会造成内存泄漏
...
* 如果文件已存在，会抛异常 FileAlreadyExistsException，其底层也是用的操作系统底层实现，和Channel方式的拷贝文件各有优劣，效率也比较高
...
* 绑定的事件类型可以有
  ...
  * read - 数据可读入时触发，有因为接收能力弱，数据暂不能读入的情况，在关闭客户端和异常情况客户端连接被强制关闭时，都会触发一个读事件，正常关闭时read的返回值为-1，强制关闭时read会报错，我们应该捕获异常后通过key.cancel来移除SelectionKeys中的这个连接
